...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        if training:
            self.train_loader_generator = self.get_loader_generator(
    ...
    def get_loader_generator(self, data_cfg, train=True):
    ...
    def get_loader(self, data_cfg, train=True):
        sampler_cfg = self.cfgs['trainer_cfg']['sampler'] if train else self.cfgs['evaluator_cfg']['sampler']
        sampler_cfg['type'] = 'TripletSampler'
        dataset = DataSet(data_cfg, train)
    ...
    def save_ckpt(self, iteration):
        ...
        if torch.distributed.get_rank() == 0:
            ...
            checkpoint = {
                ...
                'iteration': iteration}
            torch.save(checkpoint,
                       osp.join(self.save_path, 'checkpoints/{}-{:0>5}.pt'.format('Encoder', iteration)))  
            ...
            checkpoint = {
                ...
                'iteration': iteration}
            torch.save(checkpoint,
                       osp.join(self.save_path, 'checkpoints/{}-{:0>5}.pt'.format('Decoder', iteration)))  
    ...
    def _load_ckpt(self, save_name):
        ...
        self.indicator.load_state_dict(model_state_dict, strict=load_ckpt_strict)
        if self.training:
    ...
    def run_train(model):
        ...
        for inputs_generator in model.train_loader_generator:
            ipts_generator = model.inputs_pretreament(inputs_generator)
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts_generator)
