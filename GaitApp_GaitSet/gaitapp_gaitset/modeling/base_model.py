...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        if training:
            ...
            self.train_loader_original = self.get_loader_original(
                cfgs['data_cfg'], train=True)                    
        ...
        if training:
            ...
            # print("restore_hint", restore_hint)
            # print("restore_hint[0]", restore_hint[0])
            if restore_hint != 0:
                self.resume_ckpt(restore_hint[0])
                self.resume_ckpt(restore_hint[1])
    ...
    def get_loader_original(self, data_cfg, train=True):
        sampler_cfg = self.cfgs['trainer_cfg']['sampler'] if train else self.cfgs['evaluator_cfg']['sampler']
        sampler_cfg['type'] = 'TripletSampler_original'
        dataset = DataSet(data_cfg, train)
    ...
    def save_ckpt(self, iteration):
        ...
        if torch.distributed.get_rank() == 0:
            ...
            checkpoint = {
                'model': self.gaitrecog.state_dict(),
                'optimizer': self.optimizer.state_dict(),
                'scheduler': self.scheduler.state_dict(),
                'iteration': iteration}
            torch.save(checkpoint,
                       osp.join(self.save_path, 'checkpoints/{}-{:0>5}.pt'.format('Baseline', iteration)))  
    ...
    def _load_ckpt(self, save_name):
        ...
        if osp.split(save_name)[-1] == 'Encoder-20000.pt':
            print("Encoder-20000.pt is finished")
            self.encoder.load_state_dict(model_state_dict, strict=load_ckpt_strict)
        elif osp.split(save_name)[-1] == 'Decoder-20000.pt':
            print("Decoder-20000.pt is finished")
            self.decoder.load_state_dict(model_state_dict, strict=load_ckpt_strict)
    ...
    def run_train(model):
        ...
        for inputs, inputs_original in zip(model.train_loader, model.train_loader_original):
            ipts = model.inputs_pretreament(inputs)
            ipts_original = model.inputs_pretreament(inputs_original)
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts, ipts_original)
